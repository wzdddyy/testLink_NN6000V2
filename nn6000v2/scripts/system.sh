#!/usr/bin/env bash
# BUILD_DIR is expected to be absolute (normalized in update.sh).
# PROJECT_DIR provides a uniform absolute reference for all file paths.
[[ "$BUILD_DIR" == /* ]] && PROJECT_DIR="$BUILD_DIR" || PROJECT_DIR="${BASE_PATH}/../${BUILD_DIR}"

change_dnsmasq2full() {
    if ! grep -q "dnsmasq-full" "$PROJECT_DIR/include/target.mk"; then
        sed -i 's/dnsmasq/dnsmasq-full/g' "$PROJECT_DIR/include/target.mk"
    fi
}

fix_default_set() {
    if [ -d "$PROJECT_DIR/feeds/luci/collections/" ]; then
        find "$PROJECT_DIR/feeds/luci/collections/" -type f -name "Makefile" -exec sed -i "s/luci-theme-bootstrap/luci-theme-$THEME_SET/g" {} \;
    fi

    install -Dm544 "$BASE_PATH/patches/990_set_argon_primary" "$PROJECT_DIR/package/base-files/files/etc/uci-defaults/990_set_argon_primary"
    install -Dm544 "$BASE_PATH/patches/991_custom_settings" "$PROJECT_DIR/package/base-files/files/etc/uci-defaults/991_custom_settings"
    install -Dm544 "$BASE_PATH/patches/992_network_config.sh" "$PROJECT_DIR/package/base-files/files/etc/uci-defaults/992_network_config.sh"
    install -Dm544 "$BASE_PATH/patches/994_set_opkg_repos" "$PROJECT_DIR/package/base-files/files/etc/uci-defaults/994_set_opkg_repos"
    
    if [ -f "$PROJECT_DIR/package/emortal/autocore/files/tempinfo" ]; then
        if [ -f "$BASE_PATH/patches/tempinfo" ]; then
            \cp -f "$BASE_PATH/patches/tempinfo" "$PROJECT_DIR/package/emortal/autocore/files/tempinfo"
        fi
    fi
}

fix_mk_def_depends() {
    sed -i 's/libustream-mbedtls/libustream-openssl/g' "$PROJECT_DIR/include/target.mk" 2>/dev/null
    if [ -f "$PROJECT_DIR/target/linux/qualcommax/Makefile" ]; then
        sed -i 's/wpad-openssl/wpad-mesh-openssl/g' "$PROJECT_DIR/target/linux/qualcommax/Makefile"
    fi
}

fix_kconfig_recursive_dependency() {
    local file="$PROJECT_DIR/scripts/package-metadata.pl"
    if [ -f "$file" ]; then
...
update_default_lan_addr() {
    local CFG_PATH="$PROJECT_DIR/package/base-files/files/bin/config_generate"
    if [ -f "$CFG_PATH" ]; then
...
update_affinity_script() {
    local affinity_script_dir="$PROJECT_DIR/target/linux/qualcommax"
...
change_cpuusage() {
    local luci_rpc_path="$PROJECT_DIR/feeds/luci/modules/luci-base/root/usr/share/rpcd/ucode/luci"
    local qualcommax_sbin_dir="$PROJECT_DIR/target/linux/qualcommax/base-files/sbin"
    local filogic_sbin_dir="$PROJECT_DIR/target/linux/mediatek/filogic/base-files/sbin"
    ...
    local old_script_path="$PROJECT_DIR/package/base-files/files/sbin/cpuusage"
    if [ -f "$old_script_path" ]; then
        rm -f "$old_script_path"
    fi

    if [ -d "$PROJECT_DIR/target/linux/qualcommax" ]; then
        install -Dm755 "$BASE_PATH/patches/cpuusage" "$qualcommax_sbin_dir/cpuusage"
    fi
    if [ -d "$PROJECT_DIR/target/linux/mediatek" ]; then
        install -Dm755 "$BASE_PATH/patches/hnatusage" "$filogic_sbin_dir/cpuusage"
    fi
}

set_custom_task() {
    local sh_dir="$PROJECT_DIR/package/base-files/files/etc/init.d"
...
apply_passwall_tweaks() {
    local chnlist_path="$PROJECT_DIR/feeds/passwall/luci-app-passwall/root/usr/share/passwall/rules/chnlist"
    if [ -f "$chnlist_path" ]; then
        >"$chnlist_path"
    fi

    local xray_util_path="$PROJECT_DIR/feeds/passwall/luci-app-passwall/luasrc/passwall/util_xray.lua"
...
update_nss_pbuf_performance() {
    local pbuf_path="$PROJECT_DIR/package/kernel/mac80211/files/pbuf.uci"
...
set_build_signature() {
    local file="$PROJECT_DIR/feeds/luci/modules/luci-mod-status/htdocs/luci-static/resources/view/status/include/10_system.js"
...
update_nss_diag() {
    local file="$PROJECT_DIR/package/base-files/files/usr/bin/nss_diag.sh"
...
fix_compile_coremark() {
    local file="$PROJECT_DIR/feeds/packages/utils/coremark/Makefile"
    if [ -d "$(dirname "$file")" ] && [ -f "$file" ]; then
...
update_dnsmasq_conf() {
    local file="$PROJECT_DIR/package/network/services/dnsmasq/files/dhcp.conf"
    if [ -d "$(dirname "$file")" ] && [ -f "$file" ]; then
...
add_backup_info_to_sysupgrade() {
    local conf_path="$PROJECT_DIR/package/base-files/files/etc/sysupgrade.conf"
...
update_script_priority() {
    local qca_drv_path="$PROJECT_DIR/package/feeds/nss_packages/qca-nss-drv/files/qca-nss-drv.init"
    if [ -d "${qca_drv_path%/*}" ] && [ -f "$qca_drv_path" ]; then
        sed -i 's/START=.*/START=88/g' "$qca_drv_path"
    fi

    local pbuf_path="$PROJECT_DIR/package/kernel/mac80211/files/qca-nss-pbuf.init"
...
fix_rust_compile_error() {
    if [ -f "$PROJECT_DIR/feeds/packages/lang/rust/Makefile" ]; then
        sed -i 's/download-ci-llvm=true/download-ci-llvm=false/g' "$PROJECT_DIR/feeds/packages/lang/rust/Makefile"
    fi
}

update_nginx_ubus_module() {
    local makefile_path="$PROJECT_DIR/feeds/packages/net/nginx/Makefile"
...
fix_openssl_ktls() {
    local config_in="$PROJECT_DIR/package/libs/openssl/Config.in"
    if [ -f "$config_in" ]; then
...
fix_opkg_check() {
    ...
    local opkg_dir="$PROJECT_DIR/package/system/opkg"
...
install_pbr_isp() {
    local pbr_pkg_dir="$PROJECT_DIR/package/feeds/packages/pbr"
...
fix_quectel_cm() {
    local makefile_path="$PROJECT_DIR/package/feeds/packages/quectel-cm/Makefile"
    local cmake_patch_path="$PROJECT_DIR/package/feeds/packages/quectel-cm/patches/020-cmake.patch"
...
set_nginx_default_config() {
    local nginx_config_path="$PROJECT_DIR/feeds/packages/net/nginx-util/files/nginx.config"
...
EOF
    ...
    local nginx_template="$PROJECT_DIR/feeds/packages/net/nginx-util/files/uci.conf.template"
...
\tclient_body_temp_path /mnt/tmp;" "$nginx_template"
    ...
    local luci_support_script="$PROJECT_DIR/feeds/packages/net/nginx/files-luci-support/60_nginx-luci-support"
...
update_uwsgi_limit_as() {
    local cgi_io_ini="$PROJECT_DIR/feeds/packages/net/uwsgi/files-luci-support/luci-cgi_io.ini"
    local webui_ini="$PROJECT_DIR/feeds/packages/net/uwsgi/files-luci-support/luci-webui.ini"
...
remove_tweaked_packages() {
    local target_mk="$PROJECT_DIR/include/target.mk"
    if [ -f "$target_mk" ]; then
...
fix_quickstart() {
    local file_path="$PROJECT_DIR/feeds/openwrt_packages/luci-app-quickstart/luasrc/controller/istore_backend.lua"
